...
### Step 7: レビューシグナルを待ち、`github-pr-review-response` へ委譲する

PRが open だからという理由で能動ポーリングしません。新しいレビュー、review request、またはユーザーからの明示シグナルが来たら `github-pr-review-response` へ委譲します。
...
PRが存在し、実際のレビューシグナルで対応が始まるときに使う。Why: シグナル駆動の待機ルールを標準PRスキル対へ揃えるため。
...
| レビューループ | `github-pr-review-response` への委譲 | "実際のレビューシグナルが来て、全レビュー指摘へ対応しましたか？" |
