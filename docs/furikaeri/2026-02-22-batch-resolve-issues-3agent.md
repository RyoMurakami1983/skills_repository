...
- 実装済みIssueの早期Close（今回4件が放置されていた教訓）
- 残件の優先度見直し + 新規Issueのトリアージ
- 並列処理体制の検証データ蓄積
...
# 2. 各Issueの実装状況を確認
#    - コードベースに既に実装されていないか？
#    - 他のIssueやPRで解決済みではないか？

# 3. 判断と実行
#    - 実装済み → 根拠コメント付きでClose
#    - 対応可能 → 優先度ラベル付与、次回バッチ候補に
#    - 不要/重複 → 理由コメント付きでClose
#    - 大規模 → サブIssueに分割検討

# 4. 記録
#    - Close件数と理由をふりかえりドキュメントに記録
```
...
| 体制 | 構成 | 検証ポイント |
|------|------|-------------|
| 2+1 (実績あり) | Worker 2 + Reviewer 1 | ベースライン。PR #61で有効性確認済み |
| 3+1 (次回検証) | Worker 3 + Reviewer 1 | レビュー負荷が増えるか？品質低下はあるか？ |

**比較指標**:
- Wave毎の所要時間
- code-review agentの指摘件数
- false positive率（不要な指摘の割合）
- Copilot PRレビューとの差分
