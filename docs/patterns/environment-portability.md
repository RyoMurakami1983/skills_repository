...
## ランタイム選定マトリクス（Issue #123 初版）

新しい helper / deploy script を追加・改修する前に、以下の観点で候補ランタイムを比較する。

| 選択肢 | 向いている条件 | 主な利点 | 主なリスク |
|---|---|---|---|
| PowerShell ネイティブ | 主対象が Windows / PowerShell 利用者 | Windows で低摩擦、既存運用と整合 | Bash との二重実装で乖離しやすい |
| Bash ネイティブ | 主対象が macOS/Linux/WSL | Unix 環境で低摩擦 | Windows ネイティブでは利用ハードルが上がる |
| Python 標準ライブラリ単一実装 | 複数OSで同一挙動を最優先 | 実装重複を減らし挙動差を抑えやすい | Python 実行環境の前提を追加する |
| `uv` 前提スクリプト | repoメンテナ向け運用で環境統制できる | 依存管理と再現性を高く保ちやすい | 利用者向け入口には導入コストが高い場合がある |
| 共有コア + 薄いラッパー | OS別UXを残しつつロジック統一したい | UX と DRY の両立が可能 | 設計を誤ると境界が複雑化する |

### 判定ルール（推奨）

1. 利用者向け入口は **最も低摩擦** なランタイムを優先する。  
2. 同じロジックを2回以上実装しそうなら、**Python共通コア** を先に検討する。  
3. `uv` は原則として **repo運用ツール側** を優先し、利用者向け導入は慎重に判断する。  

---

## 新規実行入口を増やす前の相談チェックポイント

次のいずれかに当てはまる場合は、実装前に方針相談を行う。

- `.ps1` と `.sh` の **両方追加** が必要になりそう
- 既存スクリプトとの **挙動互換性が変わる**（引数・既定値・出力形式）
- Python / `uv` など **新しい前提依存** を利用者に要求する
- CI / ドキュメント / onboarding への **波及更新** が発生する

相談時は最低限、以下を提示する。

- 対象ユーザー（Windows中心 / cross-platform / repo maintainer）
- 候補ランタイムと比較理由
- 互換性影響と移行方法
